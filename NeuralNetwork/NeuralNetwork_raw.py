...
with tf.Session() as sess:
    ...
    for step in range(1,num_steps+1):
        ...
        if step%display_step==0 or step==1:
            ...
            print("Step"+str(step)+",Minibatch Loss"+"{:.4f}".format(loss)+",Training acc:"+"{:.4f}".format(acc))
    print("Optimization Finished!")
    #Calculate accuracy for MNIST test images
    tloss,tacc=sess.run([loss_op,accuracy],feed_dict={X:mnist.test.images,Y:mnist.test.labels})
    print("Loss" + "{:.4f}".format(tloss) + "acc:" + "{:.4f}".format(tacc))